...
export async function purchase(product, onPaid, opts = {}) {
  // Native iOS: Stars must not exist here at all. Charging for digital goods
  // outside Apple's IAP violates App Review guideline 3.1.1, and the dev mock
  // below would hand out the goods for free. Callers hide the entry point via
  // caps.stars; this is the backstop.
  if (tg.isNative) return;
  ...
  if (tg.isTelegram) {
    tg.alert(t("stars_soon", { n: stars }));
    return;
  }
  // Dev / browser: mock the purchase so the gated feature can be tested.
  if (window.confirm(t("stars_dev_confirm", { n: stars }))) onPaid();
}
