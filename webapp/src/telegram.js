...
// Close the mini app. This is what makes "delete account" final in Telegram:
// the page holds valid initData, so anything that keeps running (the 10s poll,
// the next action) would re-authenticate and register a brand-new account on
// the spot. Returns false in a plain browser, where there's nothing to close and
// the caller shows a dead-end screen instead.
export function close() {
  if (tg && isTelegram && typeof tg.close === "function") {
    try { tg.close(); return true; } catch (_) {}
  }
  return false;
}
...
// Same contract as the iOS driver: called whenever the page becomes visible
// again. In a webview (Telegram) and in a browser tab that's visibilitychange.
